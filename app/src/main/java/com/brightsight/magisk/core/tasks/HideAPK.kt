package com.brightsight.magisk.core.tasks
...
import com.brightsight.magisk.BuildConfig.APPLICATION_ID
import com.brightsight.magisk.DynAPK
...
import com.brightsight.magisk.core.Provider
import com.brightsight.magisk.core.utils.AXML
import com.brightsight.magisk.core.utils.Keygen
import com.brightsight.magisk.di.ServiceLocator
import com.brightsight.magisk.ktx.writeTo
import com.brightsight.magisk.utils.APKInstall
import com.brightsight.magisk.utils.Utils
import com.brightsight.signing.JarMap
import com.brightsight.signing.SignApk
...
object HideAPK {
    ...
    fun patch(
        ...
        try {
            val jar = com.brightsight.signing.JarMap.open(apk, true)
            ...
            com.brightsight.signing.SignApk.sign(keys.cert, keys.key, jar, FileOutputStream(out))
        } catch (e: Exception) {
            Timber.e(e)
            return false
        }
    ...
    private suspend fun patchAndHide(activity: Activity, label: String): Boolean {
        ...
        com.brightsight.magisk.utils.APKInstall.registerInstallReceiver(activity, WaitPackageReceiver(pkg, activity))
        if (!Shell.su("adb_pm_install $repack").exec().isSuccess)
            com.brightsight.magisk.utils.APKInstall.installHideResult(activity, repack)
        return true
    }
    ...
    fun restore(activity: Activity) {
        val apk = com.brightsight.magisk.DynAPK.current(activity)
        com.brightsight.magisk.utils.APKInstall.registerInstallReceiver(activity, WaitPackageReceiver(APPLICATION_ID, activity))
        Shell.su("adb_pm_install $apk").submit {
            if (!it.isSuccess)
                com.brightsight.magisk.utils.APKInstall.installHideResult(activity, apk)
        }
    }
}

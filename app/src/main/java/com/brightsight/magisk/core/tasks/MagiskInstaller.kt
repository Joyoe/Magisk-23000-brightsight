...
import com.brightsight.magisk.core.*
import com.brightsight.magisk.core.utils.MediaStoreUtils
import com.brightsight.magisk.core.utils.MediaStoreUtils.inputStream
...
import com.brightsight.magisk.ktx.withStreams
import com.brightsight.magisk.ktx.writeTo
import com.brightsight.magisk.utils.Utils
import com.brightsight.signing.SignBoot
...
abstract class MagiskInstallImpl protected constructor(
    ...
    private fun extractFiles(): Boolean {
        ...
        try {
            // Extract binaries
            if (isRunningAsStub) {
                val zf = ZipFile(com.brightsight.magisk.DynAPK.current(context))
        ...
        if (srcBoot.let { it !is SuFile || !it.isCharacter }) {
            try {
                SuFileInputStream.open(srcBoot).use {
                    if (com.brightsight.signing.SignBoot.verifySignature(it, null)) {
                        isSigned = true
        ...
        if (isSigned) {
            ...
            try {
                ...
                withStreams(src, out) { _, _ ->
                    com.brightsight.signing.SignBoot.doSignature(null, null, src, out, "/boot")
                }
            } catch (e: IOException) {
